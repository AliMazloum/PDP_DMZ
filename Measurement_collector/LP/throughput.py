...
def collect_throughput_LP_measurements(self):
            ...
            for report in measurements:
                ...
                if last_sample_time.timestamp() + sampling_speed < datetime.datetime.now().timestamp():
                    ...
                    if flows[report["flow_id"]]["total_count"] == 0:
                        report["throughput_LP_RME"] = 0
                    else:
                        report["throughput_LP_RME"] = RME(metric_value,total_data,total_count)
                    report["throughput_LP_sample_count"] = 1
                    flows[report["flow_id"]]["total_count"] = -1
                    report["metric_name"] = "throughput_LP"
                    report["throughput_LP"] = report.pop("throughput")
                    self.DB.write_measurement(report)
                    
                    if(len(total_data)>0):
                        expected_value = (1-alpha)*metric_value + alpha*(sum(total_data)/(len(total_data)))
...
def start_throughput_LP_thread(self):
    collect_throughput_LP_thread = threading.Thread(target=collect_throughput_LP_measurements(self), name="collect_throughput_LP_measurements")
