import json
import threading
def collect_throughput_MuST_measurements(self):
    ...
    while True:
        ...
        try:
            for report in measurements:
                ...
                if len(total_data) > 0 and metric_value > 0 and metric_value != total_data[len(total_data)-2]:
                    error = abs(expected_value/metric_value)
                    print(error, sampling_speed)
                elif len(total_data) > 0 and metric_value > 0 and metric_value == total_data[len(total_data)-2]:
                    error = 1
                ...
                if last_sample_time.timestamp() + sampling_speed < datetime.datetime.now().timestamp():
                    ...
                    if flows[report["flow_id"]]["total_count"] == 0:
                        report["throughput_MuST_RME"] = 0
                    else:
                        report["throughput_MuST_RME"] = RME(metric_value,total_data,total_count)
                    report["througput_MuST_sample_count"] = 1
                    flows[report["flow_id"]]["total_count"] = -1
                    report["metric_name"] = "throughput_MuST"
                    report["throughput_MuST"] = report.pop("throughput")
                    print("error: ", error, "sampling_speed: ",sampling_speed)
                    ...
                    if error > 0.95 and error < 1.05:
                        sampling_speed = min(min_sampling_speed,sampling_speed)
                        ...
                        sampling_speed = min(min_sampling_speed,2.5*sampling_speed)
                        flows[report["flow_id"]]["sampling_speed"] = sampling_speed
...
def start_throughput_MuST_thread(self):
        collect_throughput_MuST_thread = threading.Thread(target=collect_throughput_MuST_measurements(self), name="collect_throughput_MuST_measurements")
